...
void create_ServoGraphics(){
  ...
  if(multiType == FLYING_WING || multiType == TRI || multiType == BI  || multiType == DUALCOPTER || multiType == SINGLECOPTER){
    controlP5.getTab("ServoSettings").show();
    int LabelPos=100;
  ...
  if(multiType == HELI_120_CCPM || multiType == HELI_90_DEG ){
    ...
    int XPos=xServ+40;
    int YPos=yServ-0;
    Mbox  = controlP5.addCheckBox("Wbox")
          .setColorForeground(color(120)).setColorBackground(color(140))
    ...
    case MSP_SERVO_CONF:
      ...
      if (multiType == AIRPLANE ) { // Airplane OK
        if(flaps) {
          ...
          if(ServoMID[2]==4) {Cau0();}
          else if(ServoMID[2]==5) {Cau1();}
          else if(ServoMID[2]==6) {Cau2();}
          else if(ServoMID[2]==7) {Cau3();}
          else{CauClear();}
        }
      	for( i=0;i<8;i++){
